...
# Load Data
#Windows
data <- read.csv2("~/github_repos/other/Patent_Analysis/data/full_data.csv", 
...
# Windows 
table_assignee <- read.csv2("data/patentes_all_assignee_or.csv")
# Linux
table_assignee <- read.csv2("data/patentes_all_assignee_or.csv")
...
# Criar uma coluna para manter IPC's originais
data$original_code <- data$ipc
...
countries_vector <- 
  ...
  filter(n >= 4) #necessario pois aparecem muitos paises diferentes
...
#Fig. 1.2.1 - Table
table_ipc <- gather(data, str_subset(names(data), "ipc.r."), 
                    key = "ipc",
                    value = "ipc.code") %>%
  filter(ipc.code != "") %>%
...
table_ipc <- filter(table_ipc, ipc.code != "A61K-039")
...
table_ipc$ipc.rec <- ifelse(table_ipc$ipc.code %in% ipc_vector, 
                             table_ipc$ipc.code,
                             "Other")
